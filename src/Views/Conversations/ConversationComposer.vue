<template>
    <div class="conversation-compose bg-grey-lighter px-4 py-4 flex items-center " v-if="contact">
        ...
        <div class="flex-1 mx-4">
            <input class="w-full rounded-full py-2 px-2 " v-model="message" @keydown.enter="createNewConversation"
    ...
    export default {
        ...
        methods: {
            createNewConversation(e) {
                e.preventDefault();
                const newMessage = {id: this.lastMessageId, reply: this.message};
                this.$eventHub.$emit('newMessage', newMessage);
                this.message = '';
            }
        },
        watch: {
